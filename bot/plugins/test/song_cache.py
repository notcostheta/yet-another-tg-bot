...
async def cache_album(client: Client, message: Message, url: str):
    ...
    songid, album_url = await loop.run_in_executor(executor, get_album_id, url)
    print(album_url)
...
@Client.on_message(filters.command("sup") & filters.private)
@ratelimiter
